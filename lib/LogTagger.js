/**
 * TODO: change select menu text color based on option selected.
 * TODO: the "go back" function.
 * TODO: option - add color tag to name and chat or only to chat.
 * TODO: add color to actions (and maybe dice rolling).
 */
...
$(document).ready(function() {
    ...
    /**
     * Remove the hint in the input textarea when its focused
     */
	$("#input #log_text").one("focusin", function() {
...
/*
        var k = participants.keys();
        var i = 0;

        while(i < k.length) {

            var v = participants.getItem(k[i]);

            $logTemp.find(".k[i]").each(function() {
                $(this).prepend("[color="+v+"]");
